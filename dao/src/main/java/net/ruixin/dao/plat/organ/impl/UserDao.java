...
//    }
    ...
    public FastPagination getNoAdminUserList(Map map) {
        ...
        if (map.containsKey("bizAdminAllowed")) {
            //允许查看业务管理员
            sql.append(" AND RR.LEVELS ='1')");
        } else {
            sql.append(" AND RR.LEVELS IN ('1', '2'))");
        }
        if (RxStringUtils.isNotEmpty(map.get("userName"))) {
        ...
        if (RxStringUtils.isNotEmpty(map.get("defOrganName"))) {
            sql.append(" AND OO.ORGAN_NAME LIKE ? ");
            params.add("%" + map.get("defOrganName") + "%");
        }
        return super.getPaginationBySql(sql, params, map);
    }
}
