...
// 移动端的 tap 事件可自行在业务层打补丁实现
    ...
    var selectControl = {
        ...
        sync: function sync(el, keypath, instance) {
            //FixMe:————start—————select个性处理————Zp.20180620
            var $selectedOptions = $("option:selected",el);
...
// 组件是否存在某个 slot
    ...
    var Rxvm = function () {
            ...
            if (string(template) && components) {
                for (var childKey in components) {
                    if (components.hasOwnProperty(childKey)) {
                        //fixme：20181206调整子模版自动获取逻辑，解决同名前缀影响子模版的问题
                        var cmpt = components[childKey];
                        var separatorType = 1,
                            fragArr1 = template.split("<" + childKey + " "),
                            l1 = fragArr1.length,
                            cmptTplHtml = "";
                        if(l1 < 2){
                            separatorType = 2;
                        ...
                        if (l1 > 1) {
                            for (var i = 1; i < l1; i++) {
                                var frag1 = fragArr1[i],
                                    // end1 = frag1.indexOf("/>"),
                                    end2 = separatorType === 1 ? frag1.indexOf(">") : -1,
                                ...
                                if (end3 > end2) {
                                    ...
                                    fragArr1[i] = (separatorType === 1 ? " " : "") + frag1.substring(0, end2) + "/>" + frag11;
                                }else {
                                    fragArr1[i] = (separatorType === 1? " " : ">") + fragArr1[i];
        ...
        Rxvm.prototype.removeAt = function (keypath, index) {
            ...
            if(typeof index === "string"){
                var numIndex = parseInt(index,10);
                index = isNaN(numIndex) ? index : numIndex;
            }else if(!index && index !== 0 && keypath){
                var pathArr = keypath.split(".");
                var numIndex = parseInt(pathArr.pop(),10);
                index = isNaN(numIndex) ? index : numIndex;
