...
jQuery.MyCommon = {
    PageLoading: function (options) {
        ...
        var _LoadingHtml = '<div id="loadingPage" style="position:fixed;left:0;top:0;position: absolute;width:100%;height:' + _PageHeight + 'px;background:' + options.backgroundColor + ';opacity:' + options.opacity + ';filter:alpha(opacity=' + options.opacity * 100 + ');z-index:' + options.zindex + ';">' +
            '<div id="loadingTips" style="z-index:100;position: absolute; cursor1: wait; width: auto;border-color:' + options.borderColor + ';border-style:' + options.borderStyle + ';border-width:' + options.borderWidth + 'px; height:80px; line-height:80px; padding-left:20px; padding-right: 5px;border-radius:10px;  background: ' + options.backgroundColor + '; color:' + options.TipsColor + ';font-size:14px;">' +"<img src='"+RX.ctxPath+"/medias/plugin/perfectLoad/loading.gif' width='40px'/>" + '</div></div>';
        //呈现loading效果
        ...
        //计算距离，让loading提示框保持在屏幕上下左右居中loading
    ...
    hideLoading: function (option) {
        ...
        setTimeout(function () {
                loadingMask.animate(RX.browser.isIE6 ? {"filter": "Alpha(opacity=0)"} : {"opacity": 0},
                    options.delayTime,
                    function () {
                        RX.browser.isIE6 && removeIframe($(this).find(".bgiframe")[0]);
                        $(this).remove();
                    });
            },
            options.sleep);
